...
class Generator extends \yii\gii\Generator
{
    ...
    public $db = 'db';
    public $commonNs = 'app\models\common';
    ...
    public $generateCmsChildClass = true;
    public $addRuEnTrait = true;
    ...
    {
        return 'This generator generates an ActiveRecord class for the specified database table and CMS child ActiveRecord class.';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['useTablePrefix', 'generateCmsChildClass', 'useSchemaName', 'generateQuery', 'generateRelationsFromCurrentSchema'], 'boolean'],
    ...
    public function requiredTemplates()
    {
        ...
        return ['commonModel.php', 'model.php'];
    }
    ...
    {
        return array_merge(parent::stickyAttributes(), ['ns', 'db', 'baseClass', 'generateRelations', 'queryNs', 'queryBaseClass', 'useTablePrefix', 'generateQuery']);
    }
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $params = [
                ...
                'commonLabels' => $this->generateCommonLabels($tableSchema),
            ...
            $files[] = new CodeFile(
                Yii::getAlias('@' . str_replace('\\', '/', $this->commonNs)) . '/' . $modelClassName . '.php',
                $this->render('commonModel.php', $params)
            );

            if ($this->generateCmsChildClass) {
                $files[] = new CodeFile(
     ...
     * @return array the generated attribute labels (name => label)
     */
    public function generateCommonLabels($table)
    {
        $labels = [];
        foreach ($table->columns as $column) {
        ...
        foreach ($relations as $name => $relation){
            ...
            if ($generateQuery) {
                ...
                if (class_exists($queryClassRealName, true) && is_subclass_of($queryClassRealName, '\yii\db\BaseActiveRecord')) {
                    ...
                    if (strpos($activeQueryClass, $this->commonNs) === 0){
                        $activeQueryClass = StringHelper::basename($activeQueryClass);
                    }
                    $result[$name]['common'] = '\yii\db\ActiveQuery|' . $activeQueryClass;
                    $result[$name]['cms'] = '\yii\db\ActiveQuery|' . (($this->ns === $this->queryNs) ? $relation[1]: '\\' . $this->queryNs . '\\' . $relation[1]) . 'Query';;
                } else {
                    $result[$name]['common'] = '\yii\db\ActiveQuery|' . (($this->commonNs === $this->queryNs) ? $relation[1]: '\\' . $this->queryNs . '\\' . $relation[1]) . 'Query';
                    $result[$name]['cms'] = '\yii\db\ActiveQuery|' . (($this->ns === $this->queryNs) ? $relation[1]: '\\' . $this->queryNs . '\\' . $relation[1]) . 'Query';
                }
            } else {
                $result[$name]['common'] = '\yii\db\ActiveQuery';
                $result[$name]['cms'] = '\yii\db\ActiveQuery';
            }
        }
        return $result;
    }
